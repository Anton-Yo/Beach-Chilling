...
Shader "Shader Forge/Water" {
    Properties {
        ...
        _node_4781 ("node_4781", Float ) = 0
        _node_7278 ("node_7278", 2D) = "bump" {}
    ...
    SubShader {
        ...
        Pass {
            ...
            uniform sampler2D _node_7278; uniform float4 _node_7278_ST;
            UNITY_INSTANCING_BUFFER_START( Props )
                ...
                UNITY_DEFINE_INSTANCED_PROP( float, _node_4781)
...
////// Specular:
                ...
                float node_7829 = clamp(((max(0, LinearEyeDepth(SAMPLE_DEPTH_TEXTURE(_CameraDepthTexture, sceneUVs)) - _ProjectionParams.g)-(sceneUVs * 2 - 1).g)/_Distance_var),0.0,1.0);
                float2 node_8193 = _UV1;
                float3 node_3361 = UnpackNormal(tex2D(_node_7278,TRANSFORM_TEX(node_8193, _node_7278)));
                float _node_4781_var = UNITY_ACCESS_INSTANCED_PROP( Props, _node_4781 );
                ...
                float3 diffuseColor = lerp(lerp(float3(node_7829,node_7829,node_7829),node_3361.rgb,_node_4781_var),lerp(_ColorDeep_var.rgb,_ColorShallow_var.rgb,pow(1.0-max(0,dot(normalDirection, viewDirection)),clamp(_ColorFernel_var,0,4))),pow(1.0-max(0,dot(normalDirection, viewDirection)),clamp(_ColorFernelFoam_var,0,4))); // Need this for specular when using metallic
...
/// Final Color:
        ...
        Pass {
            ...
            float4 frag(VertexOutput i) : SV_Target {
                ...
                float _ColorFernel_var = UNITY_ACCESS_INSTANCED_PROP( Props, _ColorFernel );
                float _ColorFernelFoam_var = UNITY_ACCESS_INSTANCED_PROP( Props, _ColorFernelFoam );
                float3 diffColor = lerp(lerp(float3(node_7829,node_7829,node_7829),node_3361.rgb,_node_4781_var),lerp(_ColorDeep_var.rgb,_ColorShallow_var.rgb,pow(1.0-max(0,dot(normalDirection, viewDirection)),clamp(_ColorFernel_var,0,4))),pow(1.0-max(0,dot(normalDirection, viewDirection)),clamp(_ColorFernelFoam_var,0,4)));
                float specularMonochrome;
                float3 specColor;
                float _Metallic_var = UNITY_ACCESS_INSTANCED_PROP( Props, _Metallic );
