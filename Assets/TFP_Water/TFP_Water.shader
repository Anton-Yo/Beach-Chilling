    ...
    SubShader {
        ...
        Pass {
            ...
            UNITY_INSTANCING_BUFFER_START( Props )
                ...
                UNITY_DEFINE_INSTANCED_PROP( float, _DepthRefractionBlend)
                UNITY_DEFINE_INSTANCED_PROP( float, _Fresnel)
                UNITY_DEFINE_INSTANCED_PROP( float, _FogHeight)
                UNITY_DEFINE_INSTANCED_PROP( float, _ShoreFoamDepth)
                UNITY_DEFINE_INSTANCED_PROP( fixed, _UseFoam)
            ...
            float4 frag(VertexOutput i) : COLOR {
                ...
                float4 node_8168 = _Time;
                float node_3186 = 5.0;
                float2 node_8616 = ((i.uv0*objScale.r*node_3186)+node_8168.g*float2(0,0.12));
                float3 node_5329 = UnpackNormal(tex2D(_Water01,TRANSFORM_TEX(node_8616, _Water01)));
                float2 node_7137 = ((i.uv0*objScale.r*node_3186)+node_8168.g*float2(0.12,-0.12));
                ...
                float2 node_3941 = ((i.uv0*objScale.r)+node_8168.g*float2(0.05,0));
                float3 node_2013 = UnpackNormal(tex2D(_Water01,TRANSFORM_TEX(node_3941, _Water01)));
                float2 node_9630 = ((i.uv0*objScale.r)+node_8168.g*float2(-0.05,-0.05));
                ...
                float node_250 = 0.1;
                float2 node_6613 = ((i.uv0*objScale.r*node_250)+node_8168.g*float2(0.012,0.008));
                float3 node_4224 = UnpackNormal(tex2D(_Wave01,TRANSFORM_TEX(node_6613, _Wave01)));
                float2 node_1139 = ((i.uv0*objScale.r*node_250)+node_8168.g*float2(-0.01,0));
                ...
                float _DepthRefractionBlend_var = UNITY_ACCESS_INSTANCED_PROP( Props, _DepthRefractionBlend );
                float _RefractionAmount_var = UNITY_ACCESS_INSTANCED_PROP( Props, _RefractionAmount );
                float2 sceneUVs = (i.projPos.xy / i.projPos.w) + ((lerp(lerp(node_1214,node_825,0.7),node_6911,0.1).rg*clamp(saturate((sceneZ-partZ)/_DepthRefractionBlend_var),0,1))*_RefractionAmount_var);
...
////// Specular:
                ...
                float3 node_3357 = (node_2327.rgb*pow(1.0-max(0,dot(normalDirection, viewDirection)),_Fresnel_var));
                float4 node_5368 = tex2D(_Foam,TRANSFORM_TEX(node_1139, _Foam));
                float4 node_3028 = tex2D(_Foam,TRANSFORM_TEX(node_9630, _Foam));
                float _ShoreFoamDepth_var = UNITY_ACCESS_INSTANCED_PROP( Props, _ShoreFoamDepth );
                float3 _UseFoam_var = lerp( node_3357, max(node_3357,(((node_5368.rgb+node_3028.rgb)*(saturate((sceneZ-partZ)/_ShoreFoamDepth_var)*-1.0+1.0))*dot(clamp(_LightColor0.rgb,0.2,1),float3(0.3,0.59,0.11)))), UNITY_ACCESS_INSTANCED_PROP( Props, _UseFoam ) );
                float4 _FogColor_var = UNITY_ACCESS_INSTANCED_PROP( Props, _FogColor );
                float _FogHeight_var = UNITY_ACCESS_INSTANCED_PROP( Props, _FogHeight );
                float3 diffuseColor = saturate((1.0-(1.0-_UseFoam_var)*(1.0-((_LightColor0.rgb*_FogColor_var.rgb)*saturate((sceneZ-partZ)/_FogHeight_var))))); // Need this for specular when using metallic
...
/// Final Color:
                ...
                fixed4 finalRGBA = fixed4(lerp(sceneColor.rgb, finalColor,(saturate((sceneZ-partZ)/_ShoreBlend_var)*_FogColor_var.a)),1);
        ...
        Pass {
            ...
            float4 frag(VertexOutput i) : COLOR {
                ...
                float3x3 tangentTransform = float3x3( i.tangentDir, i.bitangentDir, i.normalDir);
                float3 viewDirection = normalize(_WorldSpaceCameraPos.xyz - i.posWorld.xyz);
                float4 node_2949 = _Time;
                float node_3186 = 5.0;
                float2 node_8616 = ((i.uv0*objScale.r*node_3186)+node_2949.g*float2(0,0.12));
                float3 node_5329 = UnpackNormal(tex2D(_Water01,TRANSFORM_TEX(node_8616, _Water01)));
                float2 node_7137 = ((i.uv0*objScale.r*node_3186)+node_2949.g*float2(0.12,-0.12));
                float3 node_6790 = UnpackNormal(tex2D(_Water02,TRANSFORM_TEX(node_7137, _Water02)));
                float3 node_5741 = UnpackNormal(tex2D(_Wave01,TRANSFORM_TEX(node_8616, _Wave01)));
                float3 node_7963 = UnpackNormal(tex2D(_Wave02,TRANSFORM_TEX(node_7137, _Wave02)));
                float3 node_1214 = ((node_5329.rgb+node_6790.rgb)+(node_5741.rgb+node_7963.rgb));
                float2 node_3941 = ((i.uv0*objScale.r)+node_2949.g*float2(0.05,0));
                float3 node_2013 = UnpackNormal(tex2D(_Water01,TRANSFORM_TEX(node_3941, _Water01)));
                float2 node_9630 = ((i.uv0*objScale.r)+node_2949.g*float2(-0.05,-0.05));
                float3 node_8744 = UnpackNormal(tex2D(_Water02,TRANSFORM_TEX(node_9630, _Water02)));
                float3 node_1217 = UnpackNormal(tex2D(_Wave01,TRANSFORM_TEX(node_3941, _Wave01)));
                float3 node_5571 = UnpackNormal(tex2D(_Wave02,TRANSFORM_TEX(node_9630, _Wave02)));
                float3 node_825 = ((node_2013.rgb+node_8744.rgb)+(node_1217.rgb+node_5571.rgb));
                float node_250 = 0.1;
                float2 node_6613 = ((i.uv0*objScale.r*node_250)+node_2949.g*float2(0.012,0.008));
                float3 node_4224 = UnpackNormal(tex2D(_Wave01,TRANSFORM_TEX(node_6613, _Wave01)));
                float2 node_1139 = ((i.uv0*objScale.r*node_250)+node_2949.g*float2(-0.01,0));
                float3 node_8785 = UnpackNormal(tex2D(_Wave02,TRANSFORM_TEX(node_1139, _Wave02)));
                float3 node_6911 = (node_4224.rgb+node_8785.rgb);
                float3 normalLocal = lerp(lerp(node_1214,node_825,0.7),node_6911,0.6);
                float3 normalDirection = normalize(mul( normalLocal, tangentTransform )); // Perturbed normals
...
/// Final Color:
                float3 finalColor = diffuse + specular;
                float _ShoreBlend_var = UNITY_ACCESS_INSTANCED_PROP( Props, _ShoreBlend );
                fixed4 finalRGBA = fixed4(finalColor * (saturate((sceneZ-partZ)/_ShoreBlend_var)*_FogColor_var.a),0);
                UNITY_APPLY_FOG(i.fogCoord, finalRGBA);
                return finalRGBA;
            }
            ENDCG
